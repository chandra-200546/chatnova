...
export default function AccountScreen() {
  ...
  return <main className="screen-page"><h2>Account</h2><div className="list-card"><Avatar name={name || profile?.display_name} imageUrl={avatarUrl} /><div><h4>{name || 'User'}</h4><p>{profile?.phone || ''}</p></div></div><input value={name} onChange={(e)=>setName(e.target.value)} placeholder="Display name" /><input value={bio} onChange={(e)=>setBio(e.target.value)} placeholder="Bio" /><label className="settings-item"><span>Upload profile image</span><input type="file" accept="image/*" onChange={onUpload} /></label>{uploading && <small>Uploading...</small>}<h3>Anime Avatar Gallery (1200 options)</h3><div className="avatar-grid">{options.map((url) => <button key={url} className={`avatar-pick ${avatarUrl===url?'selected':''}`} onClick={()=>onSelectAvatar(url)}><img src={url} alt="anime avatar" loading="lazy" /></button>)}</div>{savingAvatar && <small>Saving selected avatar...</small>}<button onClick={onSave}>Save Profile</button>{msg && <small>{msg}</small>}{error && <small className="danger">{error}</small>}<button className="ghost" onClick={()=>nav('/settings')}>Back</button></main>;
}
